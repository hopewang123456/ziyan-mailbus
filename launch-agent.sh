...
launch_claude_interactive() {
  if [ -f "/mailbus/tools/launch-claude-cli.py" ]; then
    RESOLVER="/mailbus/tools/launch-claude-cli.py"
    DATA_DIR="/mailbus/store"
  else
    RESOLVER="$(dirname "$0")/tools/launch-claude-cli.py"
    DATA_DIR="$(dirname "$CONFIG_FILE")"
  fi
...
launch_claude_browser() {
  ...
  if [ -f "/mailbus/tools/launch-claude-browser.py" ]; then
    resolver="/mailbus/tools/launch-claude-browser.py"
    data_dir="/mailbus/store"
  else
    resolver="$(dirname "$0")/tools/launch-claude-browser.py"
    data_dir="$(dirname "$CONFIG_FILE")"
  fi
...
launch_agent_desktop() {
  ...
  if [ -f "/mailbus/tools/launch-agent-desktop.py" ]; then
    resolver="/mailbus/tools/launch-agent-desktop.py"
    data_dir="/mailbus/store"
  else
    resolver="$(dirname "$0")/tools/launch-agent-desktop.py"
    data_dir="$(dirname "$CONFIG_FILE")"
  fi
