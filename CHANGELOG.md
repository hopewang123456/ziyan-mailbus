...
### 特性
- **mailbox-daemon (v0.5)** — Agent 侧守护进程，替代 cron 逐条推送
  - 批量处理：一次 poll 收集多条消息，批量 ack + 合并唤醒 agent
  - 任务追踪：自动创建/更新 `store/tasks/` 中的追踪链记录
  - 去重保护：内存级 `_processing_ids` 集合，防止同一消息被重复处理
  - 崩溃恢复：检测 acknowledged + not done 的消息，重启后自动接续
  - 完成回执风暴检测：✅ 开头的不再唤醒 agent（防递归）
  - 合并摘要：多条消息打包为结构化摘要，每条附带独立回复指令
