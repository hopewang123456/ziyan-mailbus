...
def verify_review_done(report: dict, *, strict: bool = False) -> Tuple[bool, Optional[str]]:
    details = report if isinstance(report, dict) else {}
    harness_verdict = details.get("harness_verdict") or (details.get("extensions") or {}).get("harness_verdict")
    if harness_verdict:
        v = str(harness_verdict).upper()
        if v == "REJECT":
            return False, "harness_verdict REJECT"
        if v == "APPROVE":
            return True, None
