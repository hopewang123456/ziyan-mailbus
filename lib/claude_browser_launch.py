...
def _http_ready_on_port(port: int) -> bool:
    """Windows 宿主探测 WSL 内 ttyd：localhost 与 WSL IP 均尝试。"""
    if _http_ready(f"http://127.0.0.1:{port}/"):
        return True
    wsl_ip = _wsl_primary_ip()
    if wsl_ip and _http_ready(f"http://{wsl_ip}:{port}/"):
        return True
    wsl = _wsl_exe()
    if wsl:
        try:
...
def ensure_claude_web(agent_key: str, data_dir: str, *, wait_seconds: int = 15) -> dict:
    ...
    try:
        ttyd_wsl = to_wsl_path(resolve_ttyd_bin())
