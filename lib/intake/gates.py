...
from ..human_queue import close_item, enqueue
from ..utils import _now_iso
from ..tracker import TaskTracker
from ..workflow.engine import on_gate_approve as task_gate_approve
from ..workflow.gate_validator import validate_approve, validate_deny
from ..workflow.registry import get_gate_def, get_workflow, load_registry
...
def on_intake_gate_approve(
    ...
    if gate_id == "contact_done":
        ...
        from ..jobs import _append_inbox_task
        ...
        actions.append({"action": "notify_lingzhang"})
    ...
    from ..human_queue import load_queue, close_item
    for item in load_queue(data_dir).get("items") or []:
        if item.get("intake_id") == intake_id and item.get("gate_id") == gate_id and item.get("status") == "pending":
            close_item(data_dir, item["id"], resolution)
            break
