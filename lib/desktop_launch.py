...
def _launch_windows_codex(codex_cfg: dict) -> None:
    ...
    if not ps_exe:
        raise RuntimeError("Windows PowerShell 不可用，无法启动 Codex Desktop")
...
""".strip()
    ...
    if result.returncode != 0:
        ...
        if "CODEX_DESKTOP_NOT_FOUND" in err:
            raise RuntimeError(
                "未检测到 Codex Desktop。请先安装：winget install Codex -s msstore"
            )
        raise RuntimeError(err or "启动 Codex Desktop 失败")
...
def _launch_linux_codex(codex_cfg: dict) -> None:
    ...
    if command == "auto":
        for cand in ("codex-app", "codex-desktop", "codex"):
            ...
            raise RuntimeError("未找到 Codex Desktop 可执行文件（codex-app / codex-desktop）")
    if "{project_dir}" in command:
        raise RuntimeError("Linux command 需由调用方替换 project_dir")
