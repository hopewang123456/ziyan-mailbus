...
from lib.composition import get_agent_profile, get_human_gate
from .tracker import TaskTracker
from lib.infra.utils import json_read, json_write


def _find_pending_item(data_dir: str, item_id: str) -> Optional[dict]:
    for item in get_human_gate(data_dir).load_queue().get("items") or []:
...
def apply_human_queue_resolution(
    ...
    if qtype == "a2a_input_required" and task_id:
        ...
        prof = get_agent_profile(reviewer) or {}
        ...
        if outcome.get("ok") and outcome.get("step_result"):
            ...
            trigger_task(data_dir, task_id, agents, __import__("lib.infra.utils", fromlist=["resolve_paths"]).resolve_paths(data_dir))
...
def resolve_human_queue_item(
    ...
    item = get_human_gate(data_dir).close_item(item_id, resolution)
    if not item:
