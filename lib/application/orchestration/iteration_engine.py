...
from lib.application.orchestration.pipeline.chain import FULL_PIPELINE_AGENTS, is_pipeline_step
...
def load_iteration_state(data_dir: str) -> dict:
    default = {
        ...
        "note": "Round2 仅在 Round1 主任务 success + 审查 audit pass 后解锁",
    }
    st = json_read(_state_path(data_dir), None)
...
def evaluate_round1_gate(data_dir: str, agents: dict = None) -> dict:
    """Round1 门禁：主任务 success + 审查 audit pass/warn → 才允许 Round2。"""
    ...
    if not task or not task.get("audit_log"):
        result["blockers"].append("主任务须由审查官写入 audit_log（POST /api/tasks/audit 或 msg-results/audit-*.json）")
...
def run_round1(data_dir: str, agents: dict) -> dict:
    ...
    if pending_audit:
        problems.append({
            ...
            "root_cause_hypothesis": "审查官未收到派单或未 POST /api/tasks/audit",
            "evidence": {
                "sample_task_ids": [t["task_id"] for t in pending_audit[:10]],
                "hint": "scan 会自动派 audit-req-* 给默认审查人；审查官须写 audit_log",
            },
        })
...
def run_round2(data_dir: str, agents: dict, diagnosis: Optional[dict] = None, force: bool = False) -> dict:
    ...
    if not force and not gate.get("round2_unlocked"):
        blocked = {
            ...
            "status": "blocked",
            "generated_at": _now_iso(),
            "reason": "Round1 未通过：须主任务 success + 审查 audit pass/warn 后才可生成 Round2 工单",
            "round1_gate": gate,
            "blockers": gate.get("blockers", []),
            "next_action": [
                "1. 完成主任务 pipeline 执行（status=success）",
                "2. 审查官处理 audit-req-* 消息并 POST /api/tasks/audit",
    ...
    for prob in diagnosis.get("problems", []):
        ...
        if cat == "inbox_backlog":
            ...
            for tid in prob.get("evidence", {}).get("task_ids", [])[:3]:
                assignee = _task_assignee(data_dir, tid) or _first_for_role("方案设计师", data_dir)
                add(
                    owner=assignee,
                    role=_role_for_agent(assignee, data_dir),
            ...
            add(
                owner=_first_for_role("开发工程师", data_dir),
                role="开发工程师",
    ...
    if inbox_agents_needing_cleanup:
        ...
        add(
            owner=_first_for_role("调度员", data_dir),
            role="调度员",
    ...
    # 固定闭环工单
    add(
        owner=_first_for_role("方案设计师", data_dir),
        role="方案设计师",
    ...
    add(
        owner=_first_for_role("审查官", data_dir),
        role="审查官",
    ...
    add(
        owner=_first_for_role("测试工程师", data_dir),
        role="测试工程师",
        title="Round2 回归验证",
        actions=[
    ...
    backlog = {
        ...
        "items": items,
        "dispatch_order": _dispatch_order(data_dir),
    ...
    protocol = {
        ...
        "loop": {
            ...
            "steps": [
                ...
                {
                    "step": 2,
                    "actor": _first_for_role("方案设计师", data_dir),
                ...
                {
                    "step": 5,
                    "actor": _first_for_role("调度员", data_dir),
        ...
        "next_iteration_trigger": (
            ...
            "由调度员发起 iteration-round-N+1 任务（task_id=iteration-{date}-r1）"
        ),
    }
...
def _role_for_agent(agent_id: str, data_dir: str = "") -> str:
    from lib.application.orchestration.pipeline.chain import agent_to_role

    return agent_to_role(agent_id, data_dir)
...
def _dispatch_order(data_dir: str = "") -> list:
    """Round2 派发顺序 — 依次取 方案设计师/调度员/开发工程师/审查官/测试工程师/调度员 候选。"""
    order_roles = [
        "方案设计师", "调度员", "开发工程师", "审查官", "测试工程师", "调度员",
    ]
    out: list = []
    for zh in order_roles:
        agent = _first_for_role(zh, data_dir)
        if agent and agent not in out:
            out.append(agent)
    return out or list(FULL_PIPELINE_AGENTS)
