...
from lib.composition import complete_llm, get_locale, llm_error_type, resolve_llm_config
...
def plan_with_llm(envelope: dict, *, data_dir: str = "", config: dict | None = None) -> dict:
    ...
    valid_rt = set(get_locale(data_dir).valid_role_types())
    for rt in chain:
        if rt not in valid_rt:
            raise PlanError("schema_invalid", f"invalid role_type={rt}")
