...
from lib.application.scan import build_queues, run_housekeeping, update_message_status
from lib.application.push.pusher import push_messages, resolve_cli_chain, resolve_cli_for_message
...
def _migrate_config(config: dict, old_version: str, new_version: str):
    ...
    migrations = [
        ...
        ("2.0.0", "2.1.0", lambda c: c.setdefault("bulletin_authors", {})),
    ]
...
def get_system_message(agent_name: str) -> dict:
    ...
    return {
        ...
        "priority": "urgent",
        "type": "system",
        "content": f"欢迎 {agent_name} 加入 mailbus 消息总线。",
        "reply_format": {
            "ack": {
...
def run_scan_once(data_dir: str, config: dict, *, quiet: bool = False) -> int:
    ...
    load_mailbus_env()
    from lib.infra.utils import named_lock
    ...
    with named_lock("mailbus-scan", blocking=False) as acquired:
        if not acquired:
            ...
            from lib.infra.utils import jsonl_append, _now_iso
            ...
            if not quiet:
                mbus_log.info("scan skipped — mailbus-scan lock held")
            return 0
...
def _run_scan_once_body(data_dir: str, config: dict, *, quiet: bool = False) -> int:
    ...
    if not agents:
        if not quiet:
            mbus_log.warn("no registered agents — run agent-add first")
        return 1
    ...
    if not quiet:
        if ack_count:
            mbus_log.info("processed %s ack(s)", ack_count)
        if fwd_count:
            mbus_log.info("processed %s forward(s)", fwd_count)
    ...
    try:
        ...
        if healed and not quiet:
            mbus_log.info("self_heal: %s", ", ".join(f"{k}={v}" for k, v in healed.items()))
    except Exception as exc:
        if not quiet:
            mbus_log.warn("scan self_heal error: %s", exc)
    ...
    try:
        ...
        if interrupted and not quiet:
            mbus_log.warn("interrupted tasks: %s", len(interrupted))
    except Exception as exc:
        if not quiet:
            mbus_log.warn("scan interrupt-detect error: %s", exc)
    ...
    if total_messages == 0:
        ...
        if not quiet:
            mbus_log.info("all agents read — no pending messages")
        return 0

    if not quiet:
        mbus_log.info("pending messages to push: %s", total_messages)
        if urgent_queue:
            mbus_log.info("urgent queue: %s messages", sum(len(v) for v in urgent_queue.values()))
        if normal_queue:
            mbus_log.info("normal queue: %s messages", sum(len(v) for v in normal_queue.values()))
        ...
        for agent_name in urgent_queue:
            if agent_name in normal_queue:
                ...
                if not quiet:
                    ...
                    mbus_log.info("%s: %s normal messages preempted by urgent", agent_name, preempted_count)
    ...
    if not quiet:
        if all_failed:
            mbus_log.warn("push failed: %s message(s)", len(all_failed))
            for fid in all_failed[:5]:
                mbus_log.warn("  failed: %s", fid)
            if len(all_failed) > 5:
                mbus_log.warn("  ... and %s more", len(all_failed) - 5)
        else:
            print("\n✓ 全部推送完成")
    ...
    if reports:
        if not quiet:
            mbus_log.info("error reports: %s", len(reports))
        for r in reports:
            tracker.update_status(r['task_id'], 'failed',
                                  {"code": r['error_code'], "reason": r['reason']})
            if not quiet:
                mbus_log.warn("  %s: [%s] %s", r['task_id'], r['error_code'], r['reason'][:60])

    heartbeat_interval = config.get("heartbeat_interval", 300)
    ops = get_context().ensure_ops()
    hb_changes = ops.heartbeat_scan(agents, config.get("agent_types", {}), data_dir, config=config,
                                 interval=heartbeat_interval)
    if hb_changes and not quiet:
        for c in hb_changes:
            if c.get("type") == "health":
                mbus_log.info("AgentMemory: %s → %s", c['old_status'], c['new_status'])
            else:
                mbus_log.info("heartbeat %s: %s → %s", c["agent"], c['old_status'], c['new_status'])

    if not quiet:
        hb_status = ops.load_status(data_dir)
        health = hb_status.get("health", {})
        am = health.get("agentmemory", {})
        if am.get("status") == "unreachable":
            mbus_log.warn("AgentMemory unreachable")
        inbox_warnings = health.get("inbox_warnings", [])
        if inbox_warnings:
            for w in inbox_warnings[:3]:
                mbus_log.warn("%s: inbox backlog %s messages", w['agent'], w['count'])
...
def cmd_heartbeat(args) -> int:
    ...
    changes = get_context().ensure_ops().heartbeat_scan(agents, agent_types, data_dir, config=config, interval=interval, missed_limit=missed_limit,
    ...
    # 健康状态
    hb_status = get_context().ensure_ops().load_status(data_dir)
    health = hb_status.get("health", {})
...
def cmd_serve(args) -> int:
    ...
    try:
        from lib.infra.internal_llm.startup import maybe_rebuild_rag_on_start
    ...
    try:
        ...
        _res = rebuild_skills_index_on_start(data_dir)
        if _res and _res.get("changes"):
            print(f"  📚 skills-index rebuilt on start ({_res['changes']} change(s))")
        else:
            print("  📚 skills-index up to date")
    except Exception as exc:
        print(f"  ⚠️ skills-index rebuild skipped: {exc}")
...
def cmd_launch(args) -> int:
    ...
    用法:
      ...
      bus.py launch --agent agent-a    启动指定 agent
...
def _push_queue(data_dir: str, config: dict, queue: dict, label: str) -> list:
    ...
    for agent_name, messages in queue.items():
        ...
        if webhook_url:
            ...
            skip_msgs = [m for m in messages if integrations.is_no_llm_notice(m)]
            if skip_msgs:
                ...
                messages = [m for m in messages if not integrations.is_no_llm_notice(m)]
                if not messages:
...
def cmd_send(args) -> int:
    ...
    if not to and domain:
        from lib.infra.utils import load_registry, resolve_domain_to_agents
    ...
    try:
        ...
        if get_integrations().is_no_llm_notice(msg_dict):
...
def cmd_archive(args) -> int:
    ...
    results = get_context().ensure_ops().archive_all(
        data_dir, agents, config.get("archive_days", 3), config.get("archive_max_messages", 300),
    )
...
def _cleanup_stale_locks(max_age: int = 300):
    ...
    # 清理 mailbus 锁文件
    for fpath in glob.glob(os.path.join(lock_root, "mailbus-*.lock")):
        try:
...
def cmd_recover(args) -> int:
    ...
    if action == "cancel":
        from lib.application.orchestration.task_recover import apply_cancel_task
        if not task_id:
            print("✗ 需要 --task-id")
            return 1
...
def cmd_iteration(args) -> int:
    ...
    from lib.application.orchestration.iteration_engine import run_round1, run_round2, run_round3, run_all, evaluate_round1_gate
