...
from lib.domain.models import Message, MsgStatus, Priority, MsgType
from lib.infra.constants import DEFAULT_CLI_MSG_MAX_CHARS
from lib.infra.utils import json_read, json_write, jsonl_append, log_error, resolve_paths, _now_iso
...
def _load_env():
    ...
    bus_dir = Path(__file__).resolve().parents[3]  # application/push → mail repo root
...
def push_messages(
    ...
    if inbox_data:
        from lib.application.scan import _get_acked_ids
    ...
    from lib.application.integrations.token_budget import load_token_budget
    from lib.composition import store_path_for_agent
    from lib.infra.utils import format_push_content_for_agent
    ...
    for cmd_template in cli_commands:
        from lib.composition import parse_model_from_push_template, try_build_push_direct
    ...
    if used_model:
        ...
        from lib.composition import should_mark_processing_on_push
        ...
        for mid in msg_ids:
            ...
            if auto_ack:
                ...
                from lib.application.scan import finalize_processing_on_push
...
def resolve_cli(
    ...
    *,
    ...
    from lib.composition import resolve_push_cli
...
def resolve_cli_for_message(
    ...
    *,
    ...
    if isinstance(action, dict) and action.get("model_tier") == "flash":
        ...
        if integ.is_pro_allowed(agent_cfg):
            alias = integ.tier_pro()
        else:
            alias = "deepseek-flash"
    elif isinstance(action, dict) and action.get("model_tier") in ("ollama", "local"):
        alias = integ.tier_ollama()
    ...
    if alias == "ollama-local" and cfg:
        settings = integ.resolve_ollama_settings(cfg, data_dir)
...
def _invoke_cli(
    ...
    *,
    ...
    from lib.composition import enqueue_launch_queue, launch_queue_prefix

    prefix = launch_queue_prefix()
    ...
    try:
        ...
        if reply_file and agent_name:
            ...
            from lib.application.orchestration.pipeline.task import is_pipeline_execute_message
            from lib.composition import push_timeout_for
            ...
            def _save_reply(proc, fpath, a_name, mids, dd, entries, timeout):
                try:
                    ...
                    from lib.application.scan import update_message_status
                    ...
                    for entry in entries or []:
                        from lib.application.orchestration.file_task_push import should_file_task_push, verify_file_task_delivery
                        from lib.application.orchestration.pipeline.task import is_pipeline_execute_message
                        ...
                        if reply_text:
                            ...
                            stall_reason = get_ops().detect_api_stall(reply_text)
                        if stall_reason and not ok:
                            from lib.application.ops.api_stall_recovery import schedule_api_stall_recovery
                    ...
                    from lib.domain.models import MsgStatus
                    from lib.application.scan import update_message_status
                    from lib.application.orchestration.file_task_push import is_executable_task
                    partial = ""
                    try:
...
def _get_unacked_ids(data_dir: str, agent_name: str, msg_ids: list) -> list:
    ...
    from lib.composition import list_unacked
