...
def rule_r2_no_auto_solution(intake: dict, data_dir: str) -> RuleMatch:
    """R2：Task A done + pursue — 禁止自动 spawn 方案设计链。"""
...
def rule_r3_spawn_solution(intake: dict, config: dict, *, manual: bool = False) -> RuleMatch:
    """R3：G1 req_to_solution approved → spawn commercial_solution design。"""
    if _gate_status(intake, "req_to_solution") != "approved":
...
def bridge_reconcile(data_dir: str, *, force: bool = False) -> dict:
    ...
    spawned = sum(
        1 for r in results
        if r.get("task_id") or r.get("spawned_task_id")
    )
    return {"status": "ok", "results": results, "spawned": spawned}
