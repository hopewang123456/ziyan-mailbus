...
def bind_workflow(task: dict, envelope: dict, *, data_dir: str) -> None:
    ...
    incoming = (ext_in.get("mailbus.workflow") or {})
    wf_e.update({
        "workflow_id": wf_id,
...
def _enqueue_gate(data_dir: str, task: dict, gate_id: str, wf: dict, *, qtype: str = "workflow_gate") -> str:
    ...
    if qtype == "llm_step_confirm":
        llm = (task.get("extensions") or {}).get("mailbus", {}).get("llm_route") or {}
...
def maybe_block_after_step(
    ...
    *,
    ...
    if mode == "llm_adaptive":
        ...
        llm_ext = (task.get("extensions") or {}).get("mailbus", {}).get("llm_route") or {}
        ...
        task.setdefault("extensions", {}).setdefault("mailbus", {})["llm_route"] = {
...
def on_gate_approve(
    ...
    tid = task.get("task_id") or ""
    _gate(data_dir).close_by_task(tid, "workflow_gate", resolution)
    _gate(data_dir).close_by_task(tid, "llm_step_confirm", resolution)
...
def on_gate_deny(
    ...
    if deny_action == "rollback_phase":
        ...
        _fsm().ensure(task)
        ziyan = task.setdefault("extensions", {}).setdefault("mailbus", {})
        ziyan.pop("llm_route", None)
        actions.append({"action": "replan_llm"})
        from lib.application.orchestration.router.dispatch import start_executing
...
def _maybe_dispatch_next(data_dir: str, task: dict) -> bool:
    ...
    chain = task.get("chain") or []
    active = _fsm().get_active_step(task)
    ...
    from lib.application.orchestration.dispatch.tier_filter import dispatch_action_from_envelope
    # LocalePort via composition
    ...
    role_zh = get_locale(data_dir).role_type_to_zh(int(rt))
    prev = chain[-1] if chain else head
    nxt = _fsm().create_next_step(
        task,
        to_role=role_zh,
        to_person=agent_id,
