...
from lib.application.workflow.registry import get_gate_def, load_registry
...
def probe_n8n_webhook(url: str, *, timeout: float = 10.0) -> dict:
    ...
    try:
        status, body = get_integrations().post_json_with_wsl_fallback(
            url,
            payload,
...
def run_video_publish_drill(
    ...
    *,
    ...
    try:
        ...
        dry = get_integrations().invoke_tool(
            data_dir,
            agent_id="mailbus",
        ...
        if mode == "live":
            ...
            live_res = get_integrations().invoke_tool(
                data_dir,
                agent_id="mailbus",
                tool_id="webhook-multi-publish",
                inputs={
