...
from lib.infra.utils import json_read, json_write, _now_iso, named_lock
...
def _register_runners():
    ...
    _JOB_RUNNERS["a2a-spec-sync"] = lambda data_dir, cfg, job: job_mod.run_a2a_spec_sync(data_dir)
...
def _scheduler_loop(data_dir: str, config: dict, sched_cfg: dict, stop: threading.Event):
    ...
    while not stop.is_set():
        ...
        for job in sched_cfg.get("jobs", []):
            ...
            if _should_run(job, js, now, current_dt, effective_interval=eff):
...
class SchedulerHub:
    ...
    def start(self):
        if not self.sched_cfg.get("enabled", True):
            mbus_log.info("scheduler disabled (config.scheduler.enabled=false)")
            return
        ...
        mbus_log.info(
            "scheduler started: %s (tick=%ss)",
            ", ".join(jobs),
            self.sched_cfg["tick_seconds"],
        )
