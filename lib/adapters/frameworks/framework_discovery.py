...
# 四端运行端适用性矩阵：docker 框架含 windows/wsl/linux/docker；claude/cursor/cline 无容器端。
# docker_sandbox 为 claude_code 实验端，默认不启用（能力预留）。
FRAMEWORK_RUN_TARGETS: dict[str, list[str]] = {
    "hermes": ["windows", "wsl", "linux", "docker"],
    "hermes_profile": ["windows", "wsl", "linux", "docker"],
    "openclaw": ["windows", "wsl", "linux", "docker"],
    "codex": ["windows", "wsl", "linux", "docker"],
    "opencode": ["windows", "wsl", "linux", "docker"],
...
RUN_TARGET_LABELS: dict[str, str] = {
    ...
    "linux": "Linux",
    "docker": "Docker",
    "docker_sandbox": "Docker Sandbox（实验）",
}

# 各 docker 框架容器内安装根（35g2/35g3 container_asset_map 的根级近似）。
# 每类资产独立挂载点的完整映射（codex workspace :ro / skills 独立挂载等）为后续项。
CONTAINER_INSTALL_ROOTS: dict[str, str] = {
    "hermes": "/home/hermes/.hermes",
    "hermes_profile": "/home/hermes/.hermes",
    "openclaw": "/workspace",
...
def framework_run_targets(framework: str) -> list[str]:
    """返回该框架可用的 run_target 端（35g2 矩阵），未知框架回落到 window 单端。"""
    fw = (framework or "").strip()
    if not fw or fw == "none":
        return ["windows"]
    return list(FRAMEWORK_RUN_TARGETS.get(fw, ["windows"]))
...
def framework_default_install_path(framework: str, *, mail_root: Path | None = None) -> str:
    ...
    if env_val:
        return env_val
    paths_key = _ENV_TO_PATHS_KEY.get(env_key, "")
    base_s = paths.get(paths_key, "") if paths_key else ""
    if not base_s:
        return ""
    base = Path(base_s)
    probe = (spec.get("probe") or "dir").strip()
    if probe.startswith("dir:") or probe.startswith("file:"):
        rel = probe.split(":", 1)[1]
...
def framework_status(*, mail_root: Path | None = None) -> dict[str, dict[str, Any]]:
    ...
    for fw, spec in reg.items():
        ...
        out[fw] = {
            ...
            "install_path": framework_default_install_path(fw, mail_root=mail_root),
