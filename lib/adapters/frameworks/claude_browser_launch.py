...
from lib.domain.errors import Fatal, MailbusError
...
from lib.infra.utils import json_read, to_wsl_path
...
def resolve_browser_port(agent_key: str, data_dir: str, browser_cfg: dict | None = None) -> int:
    ...
    if agent_cfg.get("port") not in (None, ""):
        return int(agent_cfg["port"])
...
def _run_wsl_bash(inner: str, *, timeout: int = 120) -> subprocess.CompletedProcess:
    ...
    if sys.platform == "win32":
        ...
        if not wsl:
            raise Fatal(
                "wsl.exe unavailable, cannot start Claude ttyd",
                code="wsl_unavailable",
                zh="wsl.exe 不可用，无法启动 Claude ttyd",
            )
...
def ensure_claude_web(agent_key: str, data_dir: str, *, wait_seconds: int = 15) -> dict:
    ...
    if _running_in_mailbus_docker():
        ...
        try:
            ...
            raise Fatal(str(exc), code="ttyd_not_found", zh=str(exc)) from exc
    ...
    if _running_in_mailbus_docker():
        ...
        if not enqueue_launch_queue(inner, f"claude-{agent_key}"):
            raise Fatal(
                "launch queue write failed (mailbus watchdog)",
                code="launch_queue_failed",
                zh="launch queue 写入失败（mailbus watchdog）",
            )
        ...
        if result.returncode != 0:
            err = _format_start_error(result)
            raise Fatal(
                err,
                code="claude_web_start_failed",
                zh=err,
            )
    ...
    raise Fatal(
        f"Claude ttyd not ready (port {port}). Check WSL log: {log_hint}; "
        f"if localhost fails, open via WSL IP: {url}",
        code="claude_ttyd_not_ready",
        zh=(
...
def launch_claude_browser(agent_key: str, data_dir: str) -> Dict[str, Any]:
    ...
    if not info.get("ready"):
        url = info.get("url") or agent_key
        raise Fatal(
            f"Claude browser not ready: {url}",
            code="claude_browser_not_ready",
            zh=f"Claude browser 未就绪: {url}",
        )
