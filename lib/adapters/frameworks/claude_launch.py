...
def resolve_claude_home(plat_cfg: dict, agent_name: str = "") -> str:
    """每个 agent 独立 CLAUDE_CONFIG_DIR，避免共用 ~/.claude。"""
...
def resolve_claude_workspace(agent_cfg: dict, plat_cfg: dict, agent_name: str = "") -> str:
    ...
    if agent_name and workspace and not workspace.startswith("/path/") and _is_agent_claude_workspace(workspace, agent_name):
...
def _interactive_claude_flags(
    ...
    hint = (claude_cfg.get("interactive_system_prompt") or "").strip()
    if hint:
    ...
    if extra:
        ...
        allowed_tools = (claude_cfg.get("interactive_allowed_tools") or "").strip()
        if allowed_tools:
            parts.append(f"--allowedTools {_flag_quote_value(allowed_tools)}")
    return " ".join(parts)
...
def resolve_project_dir(agent_cfg: dict, plat_cfg: dict, agent_name: str = "") -> str:
    ...
    if cwd and not cwd.startswith("/path/"):
        return _to_fs_path(cwd)
    roots = plat_cfg.get("default_project_roots") or {}
    if agent_name and roots.get(agent_name):
        root_val = str(roots[agent_name])
        if not root_val.startswith("/path/"):
            return _to_fs_path(root_val)
    default = (plat_cfg.get("default_project_dir") or "").strip()
    if default and not default.startswith("/path/"):
        return _to_fs_path(default)
    from lib.infra.env_bootstrap import mailbus_paths

    install = Path(mailbus_paths()["root"]).parent
    # Docker 容器内：mailbus 挂载在 /mailbus，parent 为容器根 /，需转 WSL 宿主机路径
    if str(install) == "/" and os.path.exists("/.dockerenv"):
        return os.environ.get("MAILBUS_PROJECT_ROOT", "<PROJECT_ROOT>")
...
def _normalize_windows_path(path: str) -> str:
    """JSON 里 <ROOT>\\ai_tools 会变成 <ROOT>\\a 转义；统一为 <ROOT>/ai_tools。"""
    raw = (path or "").strip()
    if not raw:
        return raw
...
def _build_windows_push(claude_bin: str, project_dir: str, flags: str, claude_home: str) -> str:
    ...
    if not ps_exe:
        raise Fatal(
            "PowerShell unavailable, cannot bridge Windows Claude Code",
            code="powershell_unavailable",
            zh="PowerShell 不可用，无法桥接 Windows Claude Code",
        )
    inner = (
        f"$env:USERPROFILE = Split-Path -Parent '{_ps_escape(claude_home)}'; "
        f"Set-Location '{_ps_escape(project_dir)}'; "
...
def _build_linux_push(claude_bin: str, project_dir: str, flags: str, claude_home: str) -> str:
    ...
    if _runtime_os() == "windows":
        ...
        if not wsl:
            raise Fatal(
                "wsl.exe unavailable, cannot bridge Linux Claude Code",
                code="wsl_unavailable",
                zh="wsl.exe 不可用，无法桥接 Linux Claude Code",
            )
...
def try_build_push_direct(
    ...
    *,
    ...
    """Claude Code 直连 argv（委托 lib.adapters.frameworks.direct_push，保留兼容导入）。"""
    from lib.adapters.frameworks.direct_push import try_build_push_direct as _push_direct
...
def build_interactive_command(
    ...
    *,
    ...
    inner = (
        ...
        f"AGENTMEMORY_URL=http://127.0.0.1:3111 "
        f"{shlex.quote(claude_bin if '/' in claude_bin else '/mnt/<DRIVE>/nodejs/node_global/claude')} {cli_flags}"
    )
    if _runtime_os() == "windows":
        wsl = _wsl_exe()
        return f"{shlex.quote(wsl)} -e bash -lc {_bash_single_quote(inner)}"
    ...
    if sys.platform == "win32":
        ...
        if wsl:
            try:
                ...
                pass
    raise Fatal(
        "ttyd not found. Install in WSL: sudo apt install ttyd "
        f"or ensure {ROOT}/docker-agents/codex-agent/bin/ttyd.x86_64 exists",
        code="ttyd_not_found",
        zh=(
...
def launch_interactive_desktop(agent_key: str, data_dir: str, desktop: dict) -> dict:
    ...
    if plat == "windows":
        ...
        if not launched:
            raise Fatal(
                "Failed to launch interactive Claude Code terminal",
                code="claude_terminal_launch_failed",
                zh="无法启动交互式 Claude Code 终端",
            )
...
def launch_claude_cli(agent_key: str, data_dir: str) -> dict:
    ...
    if _running_in_mailbus_docker():
        if not enqueue_launch_queue(cmd, agent_key, mode="interactive"):
            raise Fatal(
                "launch queue write failed",
                code="launch_queue_failed",
                zh="launch queue 写入失败",
            )
