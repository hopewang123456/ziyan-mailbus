...
CONFIG_SCHEMA = {
    ...
    "properties": {
        ...
        "browser_hosts": {
            "type": "array",
            "description": "浏览器白名单（IP/CIDR，如 192.168.1.0/24），空=仅本机 127.0.0.1",
            "items": {"type": "string"},
        },
        "smart_routing": {
            "type": "object",
        ...
        "mailbus_internal_llm": {
            "type": "object",
            "description": "Mailbus 内部 LLM（自动业务中转/Planner）。providers 可含 local(ollama)/remote(openai_compatible)/stub",
            "properties": {
                "enabled": {"type": "boolean", "default": True},
            ...
            "patternProperties": {
                "^[a-zA-Z][a-zA-Z0-9_-]*$": {
                    ...
                    "properties": {
                        ...
                        "enabled": {"type": "boolean", "description": "是否启用该 agent"},
                        "auth": {
                            "type": "object",
                            "description": "浏览器入口鉴权（none/token/basic/header）",
                            "properties": {
                                "mode": {"type": "string", "enum": ["none", "token", "basic", "header"]},
                                "token": {"type": "string"},
    ...
    if not isinstance(agents, dict):
        ...
        for name, cfg in agents.items():
            ...
            allowed = {"type", "role", "profile", "agent", "agent_id", "archetype", "provider",
                       "models", "webhook_url", "webhook_secret", "available", "enabled",
                       "auth", "channels", "endpoint", "runtime",
                       ...
                       "smart_routing", "native_config_path", "install_path", "run_target",
                       "host", "port", "instance_id", "topology_note",
                       "paths", "custom_paths", "skill_groups", "persona_files"}
