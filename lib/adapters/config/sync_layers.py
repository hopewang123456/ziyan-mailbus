...
from lib.infra.constants import MAILBUS_ROOT, MAILBUS_ROOT_STR, MAILBUS_DATA_STR

_MNT_RE = re.compile(r"^/mnt/([a-zA-Z])/(.*)$")


def default_use_symlink() -> bool:
    """Force link strategy everywhere (junction on Windows, symlink on POSIX).

    Windows symlink requires admin/developer-mode, conflicting with the
    privilege-free `mklink /J` junction route; `_link_dir()` already picks
    junction on Windows. Callers rely on this to decide link-vs-copy.
    """
    return True
...
def normalize_host_path(path_str: str, *, mail_root: Path | None = None) -> Path:
    ...
    # Windows drive path (E:/... or mangled E?/...): keep on Windows, map to /mnt/<d> elsewhere.
    if len(raw) >= 2 and (raw[1] == ":" or (0xF000 <= ord(raw[1]) <= 0xF0FF and raw[0].isalpha())):
        drive = raw[0].upper()
        tail = raw[2:].lstrip("/")
        if platform.system() == "Windows":
            return Path(f"{drive}:/") / tail.replace("/", "\\") if tail else Path(f"{drive}:/")
...
def mirror_rules_to_store(
    data_dir: str | Path,
    *,
    mail_root: Path | None = None,
    rules_sot: str | Path | None = None,
) -> list[str]:
    ...
    if rules_sot:
        sot = Path(rules_sot).resolve()
    else:
        cfg = json_read(os.path.join(str(data_dir), "config.json"), {})
        raw = str(((cfg.get("harness") or {}).get("rules_path") or "")).strip()
        if raw:
            candidate = Path(raw) if os.path.isabs(raw) else (MAILBUS_ROOT / raw)
            if candidate.is_dir():
                sot = candidate.resolve()
            else:
                sot = None  # type: ignore[assignment]
        else:
            sot = None  # type: ignore[assignment]
        if sot is None:
...
def build_skills_index_from_registry(
    *,
    ...
    """Build skills-index agents{} from person-index frontmatter + transport registry.

    agents{} 的 L0–L2 skills 来自 Obsidian 人物索引（profile_registry），
    并附带 reverse（skill_id → [person ids]）与 orphans（无人引用的 01-mailbus/03-shared 技能）。
    """
    ...
    index["reverse"] = _build_reverse_index(agents_out)
    index["orphans"] = _build_orphan_skills(agents_out)
    index["updated_at"] = now_utc_dt().strftime("%Y-%m-%d")
    index["schema"] = index.get("schema") or "skills-index-v2"
    index["source"] = "Obsidian person-index frontmatter + access/transport"
    return index


def write_skills_index_to_store(
    *, data_dir: str | Path | None = None, mail_root: Path | str | None = None
) -> tuple[list[str], list[str]]:
    """构建 skills-index 并幂等写回 store/agents/json/skills-index.json。

    mailbus serve 启动时自动重跑（重建 reverse/orphans 派生字段）。
    data_dir 缺省用 MAILBUS_DATA_STR。返回 (changes, errors)。
    """
    import json

    root = Path(data_dir) if data_dir else Path(MAILBUS_DATA_STR)
    index_path = root / "agents" / "json" / "skills-index.json"
    ...
    new_index = build_skills_index_from_registry(mail_root=mail_root, existing_index=existing)
    ...
    for agent_id in sorted(new_agents):
        ...
        if old_skills != new_skills:
            changes.append(f"{agent_id}: refresh skills")
    ...
    for field in ("reverse", "orphans"):
        if existing.get(field) != new_index.get(field):
...
def _build_reverse_index(agents: dict[str, Any]) -> dict[str, list[str]]:
    """skill_id → [person ids]。统计每个技能被哪些人物引用。"""
    ...
    for agent_id, entry in agents.items():
        for spec in entry.get("skills") or []:
            ...
            if not sid:
                continue
            reverse.setdefault(str(sid), []).append(agent_id)
    return {sid: sorted(persons) for sid, persons in sorted(reverse.items())}


def _build_orphan_skills(agents: dict[str, Any]) -> list[dict[str, Any]]:
    """找出 01-mailbus/012-skills 与 03-shared 中无人引用的技能目录。

    依据 Vault 中实际存在的 SKILL.md 目录（含目录名），比对 reverse 引用。
    """
    ...
    for entry in agents.values():
        for spec in entry.get("skills") or []:
            if isinstance(spec, dict) and spec.get("id"):
                used.add(str(spec["id"]))

    orphan_dirs: list[dict[str, Any]] = []
    for root, skill_name in (
        (AGENT_VAULT_ROOT / "01-mailbus" / "012-skills" / "0121-common", "shared_skill"),
        (AGENT_VAULT_ROOT / "01-mailbus" / "012-skills" / "0122-frameworks", "framework_skill"),
        (AGENT_VAULT_ROOT / "01-mailbus" / "014-team" / "0141-positions", "role_archetype"),
    ):
        if not root.is_dir():
            continue
        for child in sorted(root.iterdir()):
            ...
            if not (child / "SKILL.md").is_file():
                continue
            sid = _orphan_id_for(skill_name, child.name)
            if sid in used:
                continue
            orphan_dirs.append({"id": sid, "name": child.name, "layer": skill_name})
    return orphan_dirs


def _orphan_id_for(layer: str, name: str) -> str:
    if layer == "shared_skill":
        return "mailbus-file-protocol" if name == "mailbus-file-protocol" else name
    if layer == "framework_skill":
        return f"framework-runtime-{name.replace('_', '-')}"
    if layer == "role_archetype":
        return f"role-{name}"
    return name
...
def host_skills_dir_for_agent(agent_id: str, *, mail_root: Path | None = None) -> Path | None:
    ...
    if fw == "claude_code":
        try:
            ...
            home = Path(_to_fs_path(resolve_claude_home(plat_cfg, agent_id)))
            return home / "skills"
        except Exception:
            return None
    if fw == "codex":
