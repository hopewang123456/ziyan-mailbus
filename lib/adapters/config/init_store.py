...
def load_config_fragments(mail_root: Path | None = None) -> dict[str, Any]:
    ...
    if agents_dir.is_dir():
        from lib.adapters.config.config_files import iter_runtime_json_files
...
def build_agent_entry(
    ...
    *,
    ...
    from lib.adapters.frameworks.framework_discovery import is_framework_available
...
def ensure_ollama_local_model_alias(
    ...
    *,
    ...
    """Ensure agent_types.models.ollama-local exists (from SoT agent-types.json).

    Returns True if cfg was mutated.
    """
    models = (cfg.setdefault("agent_types", {})).setdefault("models", {})
    if isinstance(models.get("ollama-local"), dict) and models["ollama-local"]:
        return False
    root = mailbus_root(mail_root)
    sot = _read_json(root / "config" / "mailbus" / "agent-types.json", {})
    alias = ((sot.get("models") or {}).get("ollama-local") if isinstance(sot, dict) else None) or {
        "opencode": "--model ollama/{model}",
        "openclaw": "--model ollama/{model}",
        "hermes": "--provider ollama --model {model}",
        "hermes_profile": "--provider ollama --model {model}",
        "cline": "--provider ollama --model {model}",
        "claude_code": "--model {model}",
        "codex": "--model {model}",
    }
    models["ollama-local"] = dict(alias)
    return True
...
def run_init_store(
    ...
    *,
    ...
    if not quiet:
        ...
        if wf_copied:
            print(f"  workflows mirror: {len(wf_copied)} files → store/workflows/")
        if copied:
