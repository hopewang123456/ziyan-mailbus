...
from lib.infra.runtime_net import browser_base_url, rewrite_browser_host
...
def resolve_port(
    ...
    *,
    ...
    # Agent card top-level port (topology / Cockpit host:port)
...
def build_browser_url(
    ...
    *,
    ...
    """根据模板 url + 解析端口生成浏览器 URL（host 经 runtime_net 解析）。"""
    ...
    if url and port is not None:
        ...
        return rewrite_browser_host(url)
    if port is not None:
        return browser_base_url(port, path=default_path)
    return rewrite_browser_host(url.replace("{agent}", agent_key)) if url else ""
