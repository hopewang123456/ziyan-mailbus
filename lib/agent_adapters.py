...
def resolve_container(agent_cfg: dict, agent_name: str, default_service: str) -> str:
    ...
    if os.environ.get(env_key):
        return os.environ[env_key]
    from .compose_registry import resolve_compose_service
...
def model_flag(
    ...
    *,
    ...
    if model_alias == "ollama-local":
        from .ollama_routing import ollama_model_flag
        from .push_context import get_push_context
...
class A2ARemoteAdapter(BaseAdapter):
    ...
    def can_deliver_a2a(self, agent_cfg: dict) -> bool:
        from .transport.delivery import can_deliver_a2a
