...
def apply_subtasks_to_chain(task: dict, subtasks: list) -> bool:
    ...
    task.setdefault("extensions", {}).setdefault("ziyan", {})["decomposition"] = {
        "subtasks": ordered,
        "applied_at": _now_iso(),
    }
    return True
...
def block_for_clarifications(
    ...
    *,
    ...
    _append_history(task, "owner_confirmation", {"reason": reason, "human_queue_id": hq_id})
    return hq_id
...
def coding_push_allowed(task: dict, next_role_type: Optional[int], *, data_dir: str = "") -> Tuple[bool, str]:
    ...
    if next_role_type is None or int(next_role_type) not in coding_types:
        return True, ""
    ext = (task.get("extensions") or {}).get("ziyan", {}).get("decomposition")
