...
def mirror_rules_to_store(data_dir: str | Path, *, mail_root: Path | None = None) -> list[str]:
    """Copy mail/rules/**/*.md → store/rules/ (runtime mirror for hot-reload notices)."""
    root = Path(mail_root) if mail_root is not None else MAILBUS_ROOT
    ...
    copied: list[str] = []
    for src in sorted(rules_src.rglob("*.md")):
        rel = src.relative_to(rules_src)
        dst = dest / rel
...
def build_skills_index_from_registry(
    *,
    ...
    """Build skills-index agents{} from access/**/agent.json (v3 SoT)."""
    ...
    index["source"] = "access/agent.json"
    return index
