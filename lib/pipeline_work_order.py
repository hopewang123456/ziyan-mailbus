...
def write_pipeline_work_order(
    ...
    *,
    ...
    decomposition_block = ""
    try:
        rt = int(role_type) if role_type is not None else None
    except (TypeError, ValueError):
        rt = None
    if rt == 1:
        decomposition_block = """
## 设计步纪律（必读 decomposition.md）
- **模糊** → `conclusion: clarifications_needed` + human_queue，**禁止**推开发
- **复杂** → msg-results 含 `decomposition.subtasks[]`，FSM 逐步 push
- schema：`mail/rules/schemas/decomposition-result.schema.json`
"""
