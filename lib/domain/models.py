...
class MsgStatus:
    ...
    # Task-type message states (used instead of envelope status)
...
class Message:
    ...
    # Structured action / task / forward (current message schema)
...
class Inbox:
    ...
    @classmethod
    def from_dict(cls, d: dict):
        # 容错：历史损坏可能把 inbox 写成纯 messages 数组
        if isinstance(d, list):
            d = {"agent": "", "messages": d, "has_unread": bool(d)}
