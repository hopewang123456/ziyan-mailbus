...
def run_agent_cli_version_check(data_dir: str) -> int:
    ...
    if not probes:
        return 0
    if ok_count == 0:
        print("[agent-cli-version-check] warn: all probes failed (containers down?)", file=sys.stderr)
        return 0
...
def run_a2a_spec_sync(data_dir: str) -> int:
    ...
    try:
        ...
        print("[a2a-spec-sync] timeout", file=sys.stderr)
        return 1
    except Exception as exc:
        print(f"[a2a-spec-sync] error: {exc}", file=sys.stderr)
        return 1
