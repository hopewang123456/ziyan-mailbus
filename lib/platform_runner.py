...
from .utils import configure_stdio_utf8, to_wsl_path
...
def probe_http(url: str, *, timeout: float = 5.0, ok_codes: frozenset[int] = frozenset({200})) -> bool:
    try:
...
def compose_file_args() -> list[str]:
    ...
    root = Path(os.environ.get("MAILBUS_ROOT", Path(__file__).resolve().parent.parent))
