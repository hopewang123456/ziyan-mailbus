...
from .model_router import TIER_FLASH, TIER_OLLAMA, TIER_PRO, _pro_allowed

COMPLEXITY_TIERS = ("L0", "L1", "L2", "L3")
ENGINE_VERSION = "mailbus-smart-routing-v2"
...
def _routing_config_root(config: Optional[dict]) -> dict:
    """兼容旧键 squilla_router，新键 smart_routing 优先。"""
    if not config:
        return {}
    sr = config.get("smart_routing")
    if isinstance(sr, dict):
        return sr
    legacy = config.get("squilla_router")
    ...
    if root:
        ...
        if isinstance(tier_map, dict):
            ...
            for k, v in tier_map.items():
                ...
                merged[nk] = v
            out["tier_map"] = merged
    agent_sr = (agent_cfg or {}).get("smart_routing") or (agent_cfg or {}).get("squilla_router") or {}
...
def suggest_model_alias(
    ...
    *,
    ...
    if routing_cfg.get("use_ollama", True):
        from .ollama_routing import is_ollama_ready

        ollama_ready = is_ollama_ready(config, data_dir=data_dir)

    gpu_step = None
    if alias == TIER_OLLAMA and config:
        from .ollama_routing import prepare_gpu_for_ollama_push
...
def log_routing_decision(
    ...
    *,
    ...
    if not routing_cfg.get("log_decisions", False):
        return
    from .utils import _now_iso, jsonl_append
