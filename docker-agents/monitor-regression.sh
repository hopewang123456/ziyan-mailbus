...
log "========== 7. 待审计 =========="
docker exec docker-agents-mailbus-1 python3 /mailbus/tools/ops/tools/ops/flush-pending-audits.py --data-dir /mailbus/store 2>&1 | while read -r line; do echo "  $line"; done
...
" 2>/dev/null || echo "?")
if [ "$pending" = "0" ]; then pass "pending audit: 0"; else fail "pending audit: $pending (run tools/ops/flush-pending-audits.py)"; fi
