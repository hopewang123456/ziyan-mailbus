#!/bin/bash
# ============================================================
# mailbus-boot.sh v3.1 — 宿主机 legacy 启动器（DEPRECATED）
#
# Docker 部署请用: bash docker-agents/start-team.sh
...
. "${SCRIPT_DIR}/docker-agents/lib/mailbus-env.sh"

STORE_DIR="${MAILBUS_DATA}"
...
config_path = os.environ.get("MAILBUS_DATA", "") + "/config.json"
...
launch_all() {
    ...
    local idx=1
    echo "[$idx/$total] mailbus API (:${MAILBUS_API_PORT})..."
    if _check_process "bus.py (mailbus API)"; then
        echo "  ⚠️  已在运行"
    else
        cd "$MAIL_DIR"
        nohup python3 bus.py serve --host 0.0.0.0 --port ${MAILBUS_API_PORT} --data-dir "$STORE_DIR" \
