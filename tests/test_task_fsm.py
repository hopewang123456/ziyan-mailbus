...
class TestTaskFsm(unittest.TestCase):
    ...
    def test_stale_prior_step_not_wrong_agent(self):
        chain = [{"step": 5, "step_id": "s5", "to_person": "agent-g", "to_role": "开发工程师", "status": "running", "fsm_state": "awaiting_result"}]
        result = {"task_id": "t1", "agent": "agent-m", "pipeline_step": 4, "conclusion": "done"}
    ...
    def test_apply_submit_advances(self):
        ...
        out = apply_submit(task, result, agents={"agent-d": {}, "agent-a": {}})
        self.assertTrue(out["ok"])
        self.assertEqual(out["action"], "advance")
        self.assertEqual(out["next_person"], "agent-d")
        self.assertEqual(len(task["chain"]), 2)

    def test_per_step_result_file(self):
        step = {"step": 2, "step_id": "s2", "to_person": "agent-d"}
        write_step_result(self.tmp, "t1", step, {
            "agent": "agent-d", "pipeline_step": 2, "conclusion": "done",
        })
        p = step_result_path(self.tmp, "t1", "s2")
        self.assertTrue(os.path.isfile(p))
        r = read_step_result(self.tmp, "t1", step)
        self.assertEqual(r["agent"], "agent-d")

    def test_rollback_creates_redo_step(self):
        task = _task([
            {"step": 1, "step_id": "s1", "to_person": "agent-a", "to_role": "方案设计师", "status": "completed", "fsm_state": "completed"},
            {"step": 2, "step_id": "s2", "to_person": "agent-d", "to_role": "技术研究员", "status": "running", "fsm_state": "awaiting_result"},
        ])
        ensure_fsm(task)
        out = apply_rollback(task, to_step=1, reason="方案不合格")
        self.assertTrue(out["ok"])
        nxt = out["next_step"]
        self.assertEqual(nxt["to_person"], "agent-a")
        self.assertEqual(nxt["attempt"], 2)
        self.assertIn("rollback_from", nxt)

    def test_pause_v3_style(self):
        task = _task([{"step": 5, "to_person": "agent-g", "status": "running"}], task_id="game-stellar-v3-20260617")
        ensure_fsm(task)
    ...
    def test_fsm_summary(self):
        task = _task([{"step": 1, "to_person": "agent-a", "to_role": "方案设计师", "status": "running"}])
        ensure_fsm(task)
        s = fsm_summary(task)
