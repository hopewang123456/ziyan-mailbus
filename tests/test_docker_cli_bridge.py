...
from lib.platform_runner import (
...
class DockerCliBridgeTest(unittest.TestCase):
    @patch("lib.platform_runner.os.path.exists", return_value=True)
    def test_docker_sock_not_isfile(self, _exists) -> None:
        self.assertTrue(docker_sock_available())

    @patch("lib.platform_runner.os.path.isfile")
    @patch("lib.platform_runner.os.path.isdir")
    @patch("lib.platform_runner.os.path.exists")
    ...
    @patch("lib.platform_runner.os.path.isfile", return_value=True)
    @patch("lib.platform_runner.os.path.isdir", return_value=True)
    @patch("lib.platform_runner.os.path.exists", return_value=False)
