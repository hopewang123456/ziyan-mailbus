...
class TestSideAuditDefer(unittest.TestCase):
    @patch("lib.application.orchestration.pipeline.task.primary_pipeline_assignee", return_value="agent-e")
    def test_defer_when_primary_on_agent_e(self, *_):
        self.assertTrue(side_audit_deferred_for_reviewer("/tmp", "agent-e"))

    @patch("lib.application.orchestration.pipeline.task.primary_pipeline_assignee", return_value="agent-i")
    ...
    def test_codex_cli_active_for_msg_id(self):
        ...
        ps_mixed = (
            "root 1 codex exec mailbus | agent=agent-e id=audit-req-x\n"
            "root 2 codex exec mailbus | agent=agent-e id=msg-20260625-70674\n"
        )
        self.assertTrue(
            adapter.cli_active_in_ps_for(
                "agent-e", {}, ps_mixed, msg_id="msg-20260625-70674",
            )
        )
        self.assertFalse(
            adapter.cli_active_in_ps_for(
                "agent-e", {}, ps_mixed, msg_id="msg-other",
            )
        )
        self.assertTrue(adapter.cli_active_in_ps("agent-e", {}, ps_mixed))
