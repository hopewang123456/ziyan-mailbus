...
def test_index_and_search_external_tools():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        os.makedirs(os.path.join(ext, "adapters", "agent-j"))
        ...
        with open(os.path.join(ext, "grants.example.json"), "w", encoding="utf-8") as f:
            json.dump({"agent_grants": {"agent-j": ["dify-lead-enrich"]}}, f)
        with open(os.path.join(ext, "grants.json"), "w", encoding="utf-8") as f:
            json.dump({"agent_grants": {"agent-j": ["dify-lead-enrich"]}}, f)
        with open(os.path.join(ext, "adapters", "agent-j", "dify-lead-enrich.json"), "w", encoding="utf-8") as f:
            json.dump({
                "agent_id": "agent-j",
                "tool_id": "dify-lead-enrich",
                "enabled": True,
                "description": "agent-j 专用 Dify 工作流",
            }, f)

        from lib.adapters.ops.catalog_search import index_catalog, search_catalog

        n = index_catalog(store, {"agent-j": {"name": "Agent J", "role": "市场拓展", "type": "hermes_profile"}})
        assert n >= 2

        hits = search_catalog(store, "dify")
        assert any(h.get("tool_id") == "dify-lead-enrich" for h in hits)

        hits2 = search_catalog(store, "agent-j")
        assert any(h.get("agent_id") == "agent-j" for h in hits2)
