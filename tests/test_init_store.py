...
class TestInitStore(unittest.TestCase):
    ...
    def test_run_init_store_fresh(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(os.path.isfile(os.path.join(tmp, "rules", "order-intake.schema.json")))
            self.assertTrue(os.path.isfile(os.path.join(tmp, "workflows", "registry.json")))
            wf_reg = json.load(open(os.path.join(tmp, "workflows", "registry.json"), encoding="utf-8"))
            self.assertGreaterEqual(len(wf_reg.get("workflows") or {}), 6)
            ...
            for sub in ("inbox", "msg-results", "agentmemory-pending", "locks", "dispatch", "leads"):
                self.assertTrue(os.path.isdir(os.path.join(tmp, sub)), msg=sub)
            intake_path = os.path.join(tmp, "leads", "order-intake.json")
            self.assertTrue(os.path.isfile(intake_path))
