...
from lib.framework_skills import (
    AGENT_ARCHETYPES,
    framework_skill_id,
...
def sync_agent(agent: str, data_dir: str) -> dict:
    ...
    fw = ((index.get("agents") or {}).get(agent) or {}).get("framework") or "claude_code"
    arch = AGENT_ARCHETYPES.get(agent, "")
    layer_lines = [
        "> **Layer skills**（`claude_home/skills`，按需 Read，勿在回复中复述全文）：",
        "> - L0: `agent-universal`, `mailbus-file-protocol`",
        f"> - L1: `{framework_skill_id(fw)}`",
    ]
    if arch:
        layer_lines.append(f"> - L2: `role-{arch}`, `role-overlay-{agent}`")
    layer_lines.append("> Meta: `mail/docs/agent-layer-spec.md`")
