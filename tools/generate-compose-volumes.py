...
FORBIDDEN_PATTERNS = (
    ...
    re.compile(r"mail/external-tools:"),  # legacy alias sunset（现行挂载是 /mailbus/external-tools）
...
def workspace_mounts(host_prefix: str, *, mail_root: Path | None = None) -> dict[str, list[str]]:
    ...
    for agent_id, rec in sorted(load_all_agents(mail_root=root, refresh=True).items()):
        ...
        if fw == "opencode":
            ...
            lines.append(f"      - {hp}:/home/dsh/.dsh")
    return out
...
def infra_volume_lines(host_prefix: str = "", *, mail_root: Path | None = None) -> dict[str, list[str]]:
    ...
    add("dsh", paths.get("dsh_workspace", ""), "/home/dsh/.dsh", "rw")
    return lines
...
def emit_override(host_prefix: str = "", *, mail_root: Path | None = None) -> str:
    ...
    codex_web_svc = resolve_logical_service("codex-agent") or "codex-web"
    codex_review_svc = resolve_logical_service("codex-review-agent") or "codex-review"
    dsh_svc = resolve_logical_service("dsh-agent") or "dsh"
    agent_services = ["hermes", "openclaw", codex_web_svc, codex_review_svc, opencode_svc, dsh_svc, "mailbus"]
    out_lines = [
    ...
    for svc in agent_services:
        ...
        if (
            svc not in ("hermes", "mailbus", dsh_svc)
            and len(vols) <= len(shared)
            and svc not in ws
            and svc not in infra
        ):
            continue
...
def main() -> int:
    ...
    if args.check:
        ...
        if errors:
            ...
            return 2
        # override.yml 是本机私有（gitignore）；CI 无此文件时只校验已提交的 compose.yml
        if OVERRIDE_PATH.is_file():
