...
AGENTMEMORY_URL = os.environ.get(
    "AGENTMEMORY_URL",
    "http://iii-engine:3111" if os.path.exists("/mailbus") else "http://127.0.0.1:3111",
)
...
def queue_agentmemory_pending(data_dir: str, agent: str, payload: dict, rule_id: str) -> None:
    pending_dir = os.path.join(data_dir, "agentmemory-pending")
    os.makedirs(pending_dir, exist_ok=True)
    fname = f"{rule_id}-{agent}.json"
    json_write(os.path.join(pending_dir, fname), {"agent": agent, "rule_id": rule_id, "payload": payload})
