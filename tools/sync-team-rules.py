...
用法:
  ...
  python3 tools/sync-team-rules.py --data-dir store --no-notice
"""
...
def main():
    ...
    parser.add_argument(
        "--quick",
        action="store_true",
        help="启动用：只挂载/校验 rules 链接 + bulletin，跳过 AgentMemory/notice（避免 docker exec 超时）",
    )
    ...
    try:
        ...
        print(f"  rules SoT link: store/rules → Vault ({len(mirrored)} md entries)")
    except Exception as exc:
        print(f"  rules link WARN: {exc}")
        # 不阻断启动：链接失败时仍尝试后续 best-effort
        mirrored = []
    combined = NOTICE_SUMMARY + "\n\n详见 Agent/rules/mailbus/*.md（= store/rules）"
