#!/usr/bin/env python3
"""Writer for docs/question/archive/reorg-risks-by-phase.md — run after risk register updates."""
from pathlib import Path

CONTENT = r'''# Mailbus 架构重组 — 分阶段风险登记

> 计划 SoT：`C:\Users\hopew\.cursor\plans\mailbus_架构重组_bbc0d4a3.plan.md`  
> 更新：2026-06-26 · Phase 0–2 ✅；Phase 3.1–3.5 ✅；**Phase 3.6–3.8 / 4 / 5 待执行**  
...
| **P0** | 双轨 SoT（新 access/skills/rules vs 旧 adapters/roles） | 2→3 | ⚠️ sync/compose 已切；旧树仍留盘待 deprecate |
| **P0** | OpenCode 三源交付 → phantom 完成 / FSM 卡住 | 3 | ⚠️ Normalizer 已落地；需 Phase 3.8 E2E 回归 |
| **P1** | `bus.py serve` / scheduler 停服 | 1→3 | ⚠️ 部分（Phase 1 停服；未确认重启） |
| **P1** | Windows 路径污染复发（C:/E: U+F03A） | 0→3 | ✅ 已清（需 Phase 5 grep） |
| **P1** | init-store / config 合并 | 1→3 | ⚠️ 部分（launch/override 已补；base.json 仍借旧 store） |
| **P2** | symlink/junction 1920（opencode/openclaw skills） | 持续 | ⚠️ copy+rmtree 已修；需回归 |
| **P2** | 9812/9814 端口与 Dashboard 硬编码 | 3 | 🔴 开放（handlers_system 仍硬编码） |
| **P2** | 锁命名空间冲突（task lock vs scan lock） | 3 | ⚠️ `task-{id}.json` 已实现；push/scan 路径未全量接入 |
...
> 本 Cursor 会话编码/测试中暴露的风险，后续 Phase 3.3+ 或 Phase 5 处理。

| ID | 风险 | 影响 | 建议缓解 | 状态 | 阶段 |
|----|------|------|----------|------|------|
| P3-S01 | `agent_registry` 进程内缓存 | 改 agent.json 后不 reload 仍用旧数据 | `clear_agent_registry_cache()` 或重启 serve | 📋 | 3.3 |
| P3-S02 | `resolve_mailbus_path` 根推断 | 非标准 data_dir 可能不用 MAILBUS_ROOT | 统一根 + 单测 | 🔴 | 3.3+ |
| P3-S03 | `framework_skills` import 快照 | archetype 列表不随 registry 刷新 | 函数委托 registry | ⚠️ | 3.3 |
| P3-S04 | 双 registry 命名 | domain `load_registry` vs `agent_registry` 混淆 | 文档/rename | 📋 | 5 |
| P3-S05 | 路径硬编码残留 | privilege/claude_launch/handlers_tasks 等 `/mnt/e/ai_tools` | grep + MAILBUS_ROOT（#8 #28） | 🔴 | 3.3–5 |
| P3-S06 | 工具 CLI 未统一 MAILBUS_DATA | 大量 mail/tools 仍 WSL/相对 store | mailbus-send/memory-bridge ✅；tools 逐步 DEFAULT_DATA_DIR | ⚠️ | 5 |
| P3-S07 | config_schema 仅部分 v3 | 顶层 mailbus_* 无 schema | 扩展 validate | ⚠️ | 3.3 |
| P3-S08 | `--fresh` 整目录 rmtree | **不可逆**丢 inbox/tasks/results | 二次确认/runbook | 📋 | 4 |
| P3-S09 | base.json 源自旧 store | 非纯 config/ SoT，易漂移 | 拆到 config 各域 json | ⚠️ | 3.3+ |
| P3-S10 | fresh init agent 精简 | 缺 launch/profile_paths | override.json 或备份恢复 | 🔴 | 3.3–4 |
| P3-S11 | init_store↔commands 循环依赖 | 懒 import get_system_message | 抽到 lib/messages | 📋 | refactor |
| P3-S12 | org→store/roles/json 仅 init 镜像 | 运行期 org/store 双轨（#5） | fresh 覆盖或直读 org | ⚠️ | 3.3+ |
| P3-S13 | roster lingyun 字段不齐 | 缺 display/role_types | 补齐 org/json/roster | ⚠️ | org |
| P3-S14 | init 未 sync mail/rules→store/rules | 热更新/rules 镜像断链 | sync-team-rules 或文档 SoT | 🔴 | 3.3 |
| P3-S15 | serve 停服状态未知 | API/scheduler 可能未跑 | 3.8 smoke 重启 | ⚠️ | 3.8 |
| P3-S16 | Hermes .sync 路径迁移 | compose 仍挂 adapters/.sync（#27） | 3.3 compose+sync | 🔴 | 3.3 |
| P3-S17 | test_bus 语义过时 | init 现默认 13 agents | 更新测试 | ✅ | 3.8 |

**本会话已缓解**：P1-R04 ✅；agent_registry/rules_registry ✅；MAILBUS_ROOT/DATA ⚠️；init-store+pytest ✅；utils markers #40 ✅
...
| ID | 风险 | 影响 | 缓解措施 | 状态 |
|----|------|------|----------|------|
| P3-R01 | Delivery Normalizer 未落地 | OpenCode phantom done | 三源 → msg-results（#25） | ⚠️ lib/delivery_normalizer.py + scan hook |
| P3-R02 | Work Order vs msg-files 双轨 | push 上下文不一致 | work-orders SoT；过渡期兼容（#3） | ⚠️ 双写 work-orders + msg-files |
| P3-R03 | agent_registry 切换遗漏 | sync 仍读 adapters | 13 agent sync smoke ✅；validate-agent-layers 待改 | ⚠️ |
| P3-R04 | 硬编码 `/mnt/e/ai_tools/mail` | WSL/Win/容器不一致 | MAILBUS_ROOT 部分切；iteration_engine 等仍硬编码 | ⚠️ |
| P3-R05 | 9812/9814 端口分裂 | health/Dashboard 连错 | config/mailbus.json（#1） | 🔴 |
| P3-R06 | Dashboard 硬编码 workspace | 操作写错目录 | handlers_tasks ✅；handlers_system 🔴 | ⚠️ |
...
| P3-R11 | scheduler jobs 硬编码 | 漏 intake-bridge 等 | jobs.json SoT（#38） | 🔴 |
...
| P3-R14 | privilege/secrets 路径 | 权限读错根 | privilege.py ✅；secrets 路径仍相对 docker-agents | ⚠️ |
...
- pytest：registry ✅、Normalizer 三源 🔴、container paths ✅、init-store ✅、sync_layers ✅、phase34 ✅
- `bus.py status` 13 agents ✅；serve smoke ⚠️（P3-S15）
- runtime 不再以 adapters/roles 为 SoT ⚠️（sync/index 已切；validate/tests 仍引用旧树）
...
| ID | 风险 | 影响 | 缓解措施 | 状态 |
|----|------|------|----------|------|
| P4-R01 | 继续/取消/驳回未接 FSM | 按钮只改 UI | recover CLI ✅；Dashboard API 🔴；apply_cancel 已有 | ⚠️ |
...
| P4-R06 | fresh init 缺 launch 块 | Dashboard 启动 agent 失败 | override 已补四 agent；其余默认 launch 模板 | ⚠️ |
...
| P6-C01 / #44 | `mail/tools/` 180+ 冗余 | 新人不知用哪个；维护成本高 | `docs/tools-inventory.md`；22→`_incidents/`；首批归档 | ⚠️ 根目录 ~150 待下批 grep |
| P6-C02 / #45 | mail 根杂项脚本/弃用文档 | 目录臃肿 | `_run_*.sh`×4→`_archive/`；`mailbus-boot.sh` DEPRECATED 保留（start-all 委托） | ⚠️ |
| P6-C03 / #46 | 脚本目录未按 §二归类 | tools/scripts/docker-agents 混用 | `_archive/`/`_incidents/` + inventory 对照表；支持脚本 20 条 | ⚠️ |
...
| Symlink/junction 1920 | Windows opencode/openclaw skills | copy+rmtree 默认 | ⚠️ |
| 交付分裂 | patch/replies vs msg-results | 3 | ⚠️ Normalizer 已落地；E2E 待 3.8（P3-S28） |
...
| #1 | P3-R05, P3-S42 | api_port / compose 9812 |
| #32 | P3-S39 | intake bridge SoT |
| #38 | P3-R11 | scheduler jobs ✅ |
| #29 | P3-R15 | launch watchdog.json ✅ |
| #36 | — | env.template ✅ |
...
| 9 | **P6-C01 / #44** | tools 盘点归档 | `docs/tools-inventory.md` + `_incidents/` 22 项 | ⚠️ |
| 10 | **P6-C02 / #45** | mail 根杂项 | `_run_*.sh`→archive；boot 仍 DEPRECATED | ⚠️ |
| 11 | **P6-C03 / #46** | 脚本归类 | inventory 对照表 + README ×2 | ⚠️ |
...
| 44 | tools 冗余清理 | ⚠️ P6-C01 首批 |
| 45 | mail 根杂项清理 | ⚠️ P6-C02 `_run_*` |
| 46 | 脚本目录归类 | ⚠️ P6-C03 |
...
def main() -> None:
    out = Path(__file__).resolve().parent.parent.parent / "docs" / "question" / "archive" / "reorg-risks-by-phase.md"
