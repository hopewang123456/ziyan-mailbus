...
def main() -> int:
    ...
    rc, out = _run_py("tools/test-video-publish-drill.py", "--data-dir", data_dir)
    s.ok = rc == 0
    s.detail = out.splitlines()[-1] if out else f"exit {rc}"
    results.append(s)
    ...
    rc, out = _run_py("tools/smoke-platform-scout.py", "--data-dir", data_dir)
    s.ok = rc == 0
    ...
    rc, out = _run_py(
        "tools/archive-inbox-backlog.py",
        "--data-dir", data_dir,
        "--before", "2026-06-15",
    )
    s.ok = rc == 0
    ...
    if ok:
        ...
        s.optional = True
        rc, out = _run_py("tools/smoke-dashboard-api.py")
        s.ok = rc == 0
        s.detail = out.splitlines()[-1] if out else f"exit {rc}"
        results.append(s)
