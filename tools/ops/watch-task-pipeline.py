...
用法:
  python3 tools/tools/ops/watch-task-pipeline.py --task-id game-stellar-20260616
  python3 tools/tools/ops/watch-task-pipeline.py --task-id game-stellar-20260616 --interval 30 --rounds 120
  python3 tools/tools/ops/watch-task-pipeline.py --task-id game-stellar-20260616 --no-scan
"""
...
from lib.pipeline_results import read_result_from_paths, result_paths_to_try
...
LOG_DIR = os.path.join(MAIL_ROOT, "logs")
...
def _msg_result(data_dir: str, task_id: str, task: dict | None = None) -> dict | None:
    """读取当前 active step 的 step-s{step_id}.json。"""
    if not task:
        return None
    step = get_active_step(task) or (task.get("chain") or [])[-1]
    if not step:
        return None
    result = read_step_result(data_dir, task_id, step)
    if not result:
        return None
    paths = result_paths_to_try(data_dir, task_id, step)
    hit = next((p for p in paths if os.path.isfile(p)), "")
    return {
        "conclusion": result.get("conclusion"),
        "agent": result.get("agent"),
        "pipeline_step": result.get("pipeline_step"),
        "step_id": result.get("step_id"),
        "next_role": result.get("next_role"),
        "next_person": result.get("next_person"),
        "summary": (result.get("summary") or "")[:80],
        "path": os.path.basename(hit) if hit else "",
        "mtime": datetime.fromtimestamp(os.path.getmtime(hit), TZ).strftime("%H:%M:%S") if hit else "",
    }
...
def _run_scan(mail_root: str) -> None:
    try:
        subprocess.Popen(
            [sys.executable, "-m", "bus", "scan", "--data-dir", "store"],
            cwd=mail_root,
...
def one_round(
    ...
    *,
    ...
    result = _msg_result(data_dir, task_id, task)
    ...
    if signature != last_signature:
        ...
        _log(
            f"  planned 剩余 ({len(planned)}): "
            f"{' → '.join(str(x) for x in planned[:6])}{'…' if len(planned) > 6 else ''}",
            log_path,
        )
        if result:
            _log(
                f"  msg-results: file={result.get('path')} step={result.get('pipeline_step')} "
                f"agent={result.get('agent')} conclusion={result.get('conclusion')} "
