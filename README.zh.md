...
GITHUB_TOKEN=your-github-token          # 可选：platform-scout GitHub 源
...
# 或后台重启：
python tools/mailbus.py recover quick

# 5. 可选：n8n 侧车
bash docker-agents/start-n8n.sh
bash tools/_archive/setup-n8n.sh

# 6. 验收
python3 tools/_archive/run-final-acceptance.py
python3 tools/_archive/validate-order-intake.py --data-dir store
...
python tools/mailbus.py recover quick
.\tools\_archive\setup-n8n.ps1            # 或 -Reset 重建 workflow 卷
# 或：wsl bash docker-agents/start-n8n.sh

python tools\_archive\run-final-acceptance.py
```

提示：可开 **WSL mirrored 网络** 或端口转发，减少对 WSL 桥接的依赖。Docker Desktop 全团队：`wsl bash docker-agents/start-team.sh`。

### Skills / Memory 挂载（本地 Vault vs 发布）

**原则：外部知识树（如 Vault）为真源；runtime home 只是挂载点；发布仓不写本机绝对路径。**

| 层 | 本地 | 发布 / 外人 |
|----|------|-------------|
| 宿主机 | junction/symlink：`mail/skills\|rules\|plans\|docs`、各 agent `skills/`、多数 `memories/` → Vault | 仓库内相对 `skills/` · `rules/` 等 demo |
| Docker 基线 | [`docker-compose.yml`](docker-agents/docker-compose.yml) = 仓库相对 `../…` + `${ENV}` | 无 override 即可跑 demo |
| Docker 本机 | **Vault / 机器绝对路径**只写 `docker-compose.override.yml`（gitignore） | 不依赖 Vault |
| Agent 自定义 | 本机路径不进已提交 config | `examples/config.example.json` → `profile_paths.skills_dirs` / `memory_dir`；**留空 = 各 runtime 默认** |
| 环境变量模板 | [`migrate/env.template`](migrate/env.template) → `mail/.env`（勿提交） | 同左，路径改成你的机器 |

模板：复制 [`docker-agents/docker-compose.override.example.yml`](docker-agents/docker-compose.override.example.yml) → `override.yml`，把 `/path/to/…` 换成真实路径。  
Windows / WSL Docker 往往读不到 NTFS junction，容器若要用 Vault 真源须在 override 直挂。  
**不要**用已提交配置里的 `MAILBUS_*_ROOT` 指 Vault（与 junction 双源）。

#### 各 Agent 对外挂载一览

| Runtime / 服务 | 容器（或宿主机）挂载点 | 外部 SoT / 接线方式 |
|----------------|------------------------|---------------------|
| **mailbus** | `../skills` → `/mailbus/skills` 等 | 仓库 demo；override → Vault `skills/mailbus` |
| **Hermes** (`hermes`) | `${HERMES_DATA}` → `/home/hermes/.hermes`；profiles 下 `skills` / `memories` | 设 `HERMES_DATA`；可选 Vault 角色视图挂到 profiles |
| **OpenClaw** (`openclaw`) | `${OPENCLAW_WORKSPACE}` → `/workspace`；`skills` · `memory` | 设 `OPENCLAW_WORKSPACE`；override 可钉 Vault `library/openclaw` + 记忆目录 |
| **Codex · 灵霄** (`lingxiao`) | `${LINGXIAO_WORKSPACE}` → `/workspace/lingxiao`；`${CODEX_SKILLS}` → `/home/node/.codex/skills` | 设工作区 + `CODEX_SKILLS`（或 override 挂 Vault `02-agent-specific/codex`） |
| **Codex · 灵鉴** (`lingjian`) | `${LINGJIAN_WORKSPACE}` → `/workspace/lingjian`；同 Codex skills | 同灵霄 |
| **OpenCode · 大力** (`dali`) | `${OPENCODE_ROOT}` → `/workspace/opencode`（含 `skills`） | 设 `OPENCODE_ROOT`；override 可钉 Vault opencode skills |
| **Claude Code · 灵云/灵验** | 宿主机 ttyd（非 Docker 卷）；`~/.claude/skills` | 本机安装；可选 Vault → `~/.claude/skills` junction |
| **AgentMemory** | `${NODE_MODULES}` → `/node_modules` | 设 `NODE_MODULES`（需含 `@agentmemory/agentmemory`） |
| **team-pack**（可选） | `${TEAM_PACK_ROOT}/skills\|rules` → `/team-pack/…` | 有兄弟包时设 `TEAM_PACK_ROOT` |

需 mailbus 协议栈的角色视图必须含 **mailbus**；`_USE_FULL_LIBRARY` 人物挂整库（常经 `library/mailbus` junction）。
...
| 组件 | 用途 | 安装 |
|------|------|------|
| **n8n** | 多渠道发布（video drill） | `tools/_archive/setup-n8n.sh` / `tools/_archive/setup-n8n.ps1` |
...
python3 tools/_archive/validate-scheduler.py --url http://127.0.0.1:9814
python3 tools/platform-scout.py --data-dir store
python3 tools/_archive/validate-order-intake.py --data-dir store
python3 tools/_archive/run-final-acceptance.py
```
...
mailbus 将已 ack 的消息**必写** `$HERMES_DATA/shared-memory/team-memory.db`（与各 agent 启动脚本 `memory.py search` 共用），并 **best-effort** 同步到 [AgentMemory](https://github.com/rohitg00/agentmemory)（MCP 语义检索 / Codex hooks）。
...
python3 "$HERMES_DATA/scripts/memory.py" search mailbus
python3 tools/ops/check-agentmemory-persistence.py --dry-run   # 探针（加 --dry-run 跳过重启）
```
...
详见 [`docs/question/README.md`](docs/question/README.md) 与 [`docs/question/archive/mailbus-three-round-optimization.md`](docs/question/archive/mailbus-three-round-optimization.md)，以及 `rules/iteration-protocol.md`。
...
│   ├── docker-compose.yml        # 发布默认挂载（无 Vault 绝对路径）
│   ├── docker-compose.override.example.yml  # 本机 Vault 直挂模板 → 复制为 override.yml
│   ├── start-team.sh / stop-team.sh
│   └── hermes-base/ openclaw-agent/ ...
├── tools/                        # 运维/回归脚本（e2e、triage、game-lvup 等）
├── skills/ · rules/ · plans/     # 本机多为 junction → Vault；发布仓为相对 demo
├── store/                        # 运行时数据目录（gitignore，不提交）
├── tests/                        # 测试套件
├── docs/
│   ├── platform.html             # Web 管理界面（独立 HTML，零依赖）
│   ├── skills-memory-paths.md    # Skills/Memory 挂载与发布契约
│   ├── quickstart.md
│   └── message-format.md
├── examples/
│   └── config.example.json       # skills_dirs / memory_dir 示例；留空走 runtime 默认
├── ARCHITECTURE.md
├── README.md
├── CHANGELOG.md
├── LICENSE
